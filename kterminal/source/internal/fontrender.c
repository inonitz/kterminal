...
/* Need static assert to make sure this is 1 byte alignment, and 3 bytes total */
...
position2D_t functor_name( \
    ...
    uint8_t  const* charBitmap    = &font->GlyphBitmaps[(uint32_t)idx * font->GlyphHeight * font->GlyphBytesWidth]; \
    ...
    /* hwfg & hwbg should be passed to this function as arguments, rather than computing them every time */ \
    pixel_type hwfg = (pixel_type)packColourToHardwareFormat(&canvas->m_pixelInfo, fg); \
    pixel_type hwbg = (pixel_type)packColourToHardwareFormat(&canvas->m_pixelInfo, bg); \
    \
    \
    ...
    for (uint32_t v = 0; v < font->GlyphHeight; ++v) { \
        ...
        for (uint32_t h = 0; h < glyphWidthFullBytes; ++h) { \
            ...
            fbGlyphRowAddr[0] = (currentByte & 0b10000000) ? hwfg : hwbg; \
            fbGlyphRowAddr[1] = (currentByte & 0b01000000) ? hwfg : hwbg; \
            fbGlyphRowAddr[2] = (currentByte & 0b00100000) ? hwfg : hwbg; \
            fbGlyphRowAddr[3] = (currentByte & 0b00010000) ? hwfg : hwbg; \
            fbGlyphRowAddr[4] = (currentByte & 0b00001000) ? hwfg : hwbg; \
            fbGlyphRowAddr[5] = (currentByte & 0b00000100) ? hwfg : hwbg; \
            fbGlyphRowAddr[6] = (currentByte & 0b00000010) ? hwfg : hwbg; \
            fbGlyphRowAddr[7] = (currentByte & 0b00000001) ? hwfg : hwbg; \
            fbGlyphRowAddr += 8; \
        } \
        \
        currentByte = charBitmap[glyphWidthFullBytes]; \
        for(uint8_t bit = 0; bit < glyphWidthExtraBits; ++bit) { \
            fbGlyphRowAddr[bit] = (currentByte & (0b10000000 >> bit)) ? hwfg : hwbg; \
        } \
        fbRowAddr  += fbStride; \
...
TEMPLATE_GENERATE_CHAR_BLIT_FUNCTOR(bmfont_blit_uint16_t, uint16_t, 1)
TEMPLATE_GENERATE_CHAR_BLIT_FUNCTOR(bmfont_blit_uint32_t, uint32_t, 1)
// TEMPLATE_GENERATE_CHAR_BLIT_FUNCTOR(bmfont_blit_uint32_t, rgb24, 1)
